...
class BinaryTree:
    ...
    def _search(self, node, x):
        nodeIT = node
        if nodeIT.elem is x or node is None:
            return nodeIT
        elif nodeIT.elem < x:
            return self._search(nodeIT.right, x)
        elif nodeIT.elem > x:
            return self._search(nodeIT.left, x)

    def _searchwhile(self, x):
        node = self._root
        while node:
            if node.elem == x:
                return node
    ...
    def _insert(self, node: BinaryNode, x) -> BinaryNode:
        if node is None:
            return BinaryNode(x)
        if node.elem == x:
            print("Error: element already exists in the tree")
            return node
    ...
    def _remove(self, node, x):
        ...
        if x < node.elem:
            ...
            if node.left is None and node.tight is None:
