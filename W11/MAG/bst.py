...
class BinarySearchTree(BinaryTree):
    ...
    def in_order2_list(self):
        result = []
        self._in_order2_list(self._root, result)
        return result

    def _in_order2_list(self, node, lista):
        if node.right is None and node.left is None:
            return None
        self._in_order2_list(node.right, lista)
        lista.append(node.elem)
        self._in_order2_list(node.left, lista)
...
if __name__ == "__main__":
    ...
    arbol1 = (50, 48, 70, 30, 65, 90, 20, 32, 67, 98, 25, 31, 35, 66, 69, 94, 99)
    arbol2 = (25, 20, 36, 10, 22, 30, 40, 5, 12, 28, 38, 48)
    a = BinarySearchTree()
    for i in arbol1:
        a.insert(i)
    a.draw()
